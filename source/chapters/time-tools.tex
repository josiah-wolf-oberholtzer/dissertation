...
\eblettrinedbl{C}{onsort implements a model of composition} which relies on a
number of interrelated but distinct approaches to working with musical time.
For example, Abjad's \emph{Timespan} suggests a \enquote{coarse} approach to
musical time. These objects represent arbitrary durated events on a timeline,
without respect for score hierarchy or meter. They are well-suited for modeling
...
timespan object beginning at the offset \sfrac{1}{4} and continues up until the
offset \sfrac{3}{2}:
...
timespan, from \sfrac{1}{4} to \sfrac{11}{8}:
...
unsynchronized 1-length groups of \sfrac{1}{4}-duration timespans, separated by
\sfrac{1}{4}-duration silences, creating the appearance of synchronization
across voices:
...
from a series of \sfrac{1}{4}-durations to \sfrac{1}{4}, \sfrac{2}{4},
\sfrac{3}{4}, \sfrac{4}{4} reveals the timespan-maker's voice-wrapping
behavior. Note how \enquote{Voice 1}'s timespans receive the durations
\sfrac{1}{4}, \sfrac{2}{4}, \sfrac{3}{4}, \sfrac{4}{4}, \sfrac{1}{4} and
\sfrac{2}{4}. \enquote{Voice 2} continues the duration sequence with
\sfrac{3}{4}, \sfrac{4}{4} and so forth. The playing-duration talea continues
...
\sfrac{13}{4} to \sfrac{4}{1}, should -- according to the following talea
timespan-maker's groupings pattern -- begin a group of length 2. Because that
grouping would run beyond the target timespan's \sfrac{19}{4} stop offset, the
...
the duration of each timespan follows the timespan-maker's \sfrac{1}{4},
\sfrac{2}{4}, \sfrac{3}{4}, \sfrac{4}{4}, \sfrac{5}{4} pattern not from
...
of the next group. Therefore the second group starts at \sfrac{1}{1} because
the first silence duration is \sfrac{4}{8} and the first group stops at
\sfrac{1}{2}. Similarly, the third group starts at \sfrac{5}{2} (\sfrac{20}{8})
because the second silence duration is \sfrac{7}{8} and the second group ended
at \sfrac{13}{8}. This silence \emph{stepping} can also be calculated from the
...
is missing a fourth timespan. That timespan, with a duration of \sfrac{5}{8},
and an initial silence of \sfrac{2}{8} measured from the offset \sfrac{4}{1},
would have extended to \sfrac{39}{8} -- \sfrac{1}{8} more than the target
timespan's \sfrac{19}{4} stop offset:
...
offsets \sfrac{10}{1}, \sfrac{15}{1} and \sfrac{25}{1}, for example. This is
because a new timespan begins in \enquote{Voice 1} at both \sfrac{10}{1} and
\sfrac{25}{1}, and another new timespan begins in \enquote{Voice 2} at
\sfrac{15}{1}:
...
following a \sfrac{5}{1}-duration timespan:
...
between the offsets \sfrac{17}{1} and \sfrac{58}{1}:
...
\sfrac{4}{16} from \sfrac{2}{8} or \sfrac{6}{8} from \sfrac{3}{4} and to
...
comprised of a single \sfrac{1}{16} duration:
...
\noindent Extending the talea counts to a sequence of alternating \sfrac{1}{16}
and \sfrac{1}{8} notes produces more complex results. Note how the \sfrac{1}{8}
durations break over the boundaries of the \sfrac{3}{16} measure, but remain
...
count to every second and third input division. This causes the \sfrac{4}{8}
and \sfrac{3}{16} as well as the \sfrac{5}{8} and \sfrac{2}{4} divisions to
become tuplets, each with a pre-prolated contents duration \sfrac{1}{16} longer
than their prolated duration. In other words, the \sfrac{4}{8} division
contains a tuplet whose contents sum to \sfrac{9}{16}, but scaled into a
duration of \sfrac{8}{16}. Likewise, the \sfrac{3}{16} division contains a
tuplet whose contents sum to \sfrac{4}{16}, but scaled into a duration of
\sfrac{3}{16}. Note how this tupletting causes the pattern of split and tied
talea durations to shift. In the previous example, the second instance of the
talea's \sfrac{3}{16} duration occurred entirely during the \sfrac{3}{16}
division. Here, that same \sfrac{3}{16} talea duration begins on the final
\sfrac{1}{16} of the \sfrac{4}{8} division's 9:8 tuplet, tied into the 4:3
...
rhythm-maker incises a single \sfrac{1}{16} rest at the end of each division:
...
divisions of their output. The following rhythm-maker cuts \sfrac{1}{8} rests
...
2-length pattern produces divisions filled alternatingly with \sfrac{1}{8} and
\sfrac{1}{16} notes. Note that the \sfrac{3}{16} measure is filled with a
dotted \sfrac{1}{8} note:
...
complex output. Note here how the \sfrac{5}{8} division is rendered as a
tuplet:
...
\sfrac{1}{8} notes, every other division is extended by one count, causing the
\sfrac{4}{8} and \sfrac{2}{4} divisions to be rendered as 5:4 tuplets, and the
\sfrac{4}{16} division to be rendered as a 3:2 tuplet:
...
Consider the following \sfrac{6}{8} meter and its graph representation:
...
metrical grouping in a \sfrac{6}{8} meter. Tracing the leftmost and rightmost
arrows down through the topmost node's children gives the offsets 0 and
\sfrac{3}{4}: the first downbeat and next downbeat in a \sfrac{6}{8} meter.
Offsets 0 and \sfrac{3}{4} also have the strongest accent weights as they occur
as either the start offset or stop offset of nodes at three levels of hierarchy
in the rhythm tree. At the second level the \sfrac{6}{8} grouping divides into
two \sfrac{3}{8} groupings, following common practice expectations: metrical
groupings tend to subdivide into groups of two before they subdivide into
groups of three.\footnote{Consider a \sfrac{12}{8} meter. Western musicians
...
offset of \sfrac{3}{8}, which also occurs in the third level, giving
\sfrac{3}{8} a weight of two. The third level contains the \sfrac{1}{8}
duration beats, grouped by their parents in the second level into two groups
three \sfrac{1}{8} duration nodes. The offsets \sfrac{1}{8}, \sfrac{1}{4},
\sfrac{1}{2} and \sfrac{5}{8} are not shared by any nodes except at the lowest
...
\sfrac{3}{4} meter consists of a top-level \sfrac{3}{4} metrical grouping
divided into three \sfrac{1}{4} duration beats:
...
\noindent By default, a \sfrac{7}{8} meter subdivides its top-level metrical
grouping into \sfrac{3}{8}+\sfrac{2}{8}+\sfrac{2}{8} groupings:
...
\noindent A \sfrac{12}{8} meter subdivides into four \sfrac{3}{8} duration
groupings, each containing three \sfrac{1}{8} duration beats:
...
of \sfrac{4}{4} generates a top-level rhythmic grouping with a duration of
\sfrac{4}{4} and four \sfrac{1}{4} beats as children:\footnote{A \enquote{flat}
\sfrac{4}{4} metrical structure is useful for meter rewriting as it allows the
meter rewriting algorithm to ignore many common rhythmic idioms like
\sfrac{1}{4}+\sfrac{1}{2}+\sfrac{1}{4} and \sfrac{1}{4}+\sfrac{3}{4}.}
...
example, a \sfrac{4}{4} meter which strongly emphasizes beat three is possible
by subdividing the top-level \sfrac{4}{4} metrical grouping into two
\sfrac{2}{4} duration groupings, which are then subdivided each into two
\sfrac{1}{4} duration beats. Such a metrical structure effectively treats
\sfrac{4}{4} as identical to \sfrac{2}{2}:
...
\sfrac{4}{4} meter which divides into two parts, with the first part dividing
...
For example, the offsets outlined by the root node of a \sfrac{6}{8} meter can
...
would be considered valid in the context of \sfrac{6}{8} because it aligns to
the offsets outlined at depth-0 of the meter's rhythm-tree. In contrast, a
logical tie two quarters in duration starting at \sfrac{1}{8} -- therefore
outlining the timespan of \sfrac{1}{8}:\sfrac{5}{8} -- would be considered
...
depth of 1 in a \sfrac{6}{8} meter we finally find an offset intersecting the
timespan of the misaligned \sfrac{1}{8}:\sfrac{5}{8} logical tie at the offset
\sfrac{3}{8}:
...
\noindent Rewriting the contents of the \sfrac{6}{8} measure against a
\sfrac{6}{8} meter splits the inner half-note at the \sfrac{3}{8} offset
boundary:
...
a \sfrac{4}{4} metrical structure: \sfrac{0}{4}, \sfrac{1}{4}, \sfrac{2}{4},
\sfrac{3}{4} or \sfrac{4}{4}:
...
\noindent Rewriting the inner measure against a \sfrac{4}{4} meter breaks the
inner logical tie at \sfrac{1}{4} and \sfrac{3}{4}, slightly improving
readability:
...
\noindent Rewriting the same measure against a \sfrac{2}{2} meter breaks the
inner logical tie at the offset \sfrac{1}{2}, as two double-dotted quarter
notes. While identical to \sfrac{4}{4} in duration, \sfrac{2}{2} strongly
...
following unrewritten \sfrac{3}{4} measure:
...
\enquote{3-ness} inherent to a \sfrac{9}{8} meter. The nodes with a depth of 1
in the following \sfrac{9}{8} meter's rhythm-tree -- the three \sfrac{3}{8}
inner nodes marked by triangles -- outline the offsets \sfrac{0}{8},
\sfrac{3}{8}, \sfrac{6}{8} and \sfrac{9}{8}. When rewriting a \sfrac{9}{8}
...
half-note in the above \sfrac{9}{8} measure has been split into two
quarter-notes, tied together. The second of these quarter-notes begins at
\sfrac{6}{8}, therefore aligning with the start of the third \sfrac{3}{8} node
at depth 1 in the \sfrac{9}{8} meter's rhythm tree. Likewise, the first half of
the split half-note ends at the same \sfrac{6}{8} offset. However, the initial
...
outlined by the offsets \sfrac{3}{8} and \sfrac{6}{8}:
...
\sfrac{3}{8} offset. Because the initial half note aligned at the \sfrac{0}{8}
offset -- as outlined by the root node in the \sfrac{9}{8} meter's rhythm tree
--, but not at the \sfrac{3}{8} offset -- as outlined by the first and second
\sfrac{3}{8}-duration nodes at depth 1 of the same meter's rhythm tree --, it
...
identical durations, such as \sfrac{3}{4} and \sfrac{6}{8}, or between various
possible representations of other prime-numerator meters like \sfrac{5}{4},
\sfrac{7}{8} and so forth. The following series of \sfrac{3}{4} measures
contain rhythms which emphasize \sfrac{3}{4}-ness or \sfrac{6}{8}-ness to
...
\noindent Rewriting under a \sfrac{3}{4} meter with a boundary depth of 1
forces emphasis of the offsets found in the \sfrac{3}{4} meter's depth-1 group
of three quarter-duration nodes: \sfrac{0}{4}, \sfrac{1}{4}, \sfrac{2}{4} and
\sfrac{3}{4}. Both the outer measures' contents align to these offsets, but the
...
\noindent Conversely, rewriting with a \sfrac{6}{8} meter -- again with a
boundary depth of 1 -- forces emphasis of the offsets outlined by
\sfrac{6}{8}'s two depth-1 \sfrac{3}{8}-duration nodes: \sfrac{0}{8},
\sfrac{3}{8} and \sfrac{6}{8}. Only the inner measure's contents align
perfectly these offsets, so the outer measures are rewritten, better
demonstrating \sfrac{6}{8}'s duple-ness:
...
\noindent Note that the offset \sfrac{1}{2} shows a count of 4. This is because
\sfrac{1}{2} acts as both the start or stop offset for four separate leaves in
the score.
...
\sfrac{3}{8} offset in a \sfrac{6}{8} measure is less strong than its downbeat
but still stronger than the offsets at \sfrac{1}{8}, \sfrac{2}{8}, \sfrac{4}{8}
or \sfrac{5}{8}. However, the default model of a \sfrac{6}{8} meter makes no
explicit reference to offsets such as \sfrac{1}{16} or \sfrac{3}{16}. A common
practice understanding of meter tells us that these offsets should be taken as
less accented than those at \sfrac{1}{8}, \sfrac{2}{8}, \sfrac{3}{8} and so
forth. They effectively represent an even lower level of leaves on the
rhythm-tree for \sfrac{6}{8}. Likewise, offsets with a denominator of 32 or 64
...
Here a metric accent kernel is generated from a \sfrac{4}{4} meter, extending
its denominator to a \emph{limit} of \sfrac{1}{16}. The weights at each offset
...
\noindent The \sfrac{4}{4} metric accent kernel can be called against an offset
...
\noindent Now consider the metric accent kernels for \sfrac{3}{4}, \sfrac{7}{8}
and \sfrac{5}{4} meters:
...
performed only \sfrac{4}{4} meters should be selected as they perfectly align
...
\noindent If we change the input offsets to no longer outline only \sfrac{4}{4}
meters, the meter fitting process will arrive at a different solution. Note how
the following fitted meters emphasize the \sfrac{3}{4} and \sfrac{5}{4}
durations inherent to the new input offsets. The initial \sfrac{3}{4} fitted
meter perfectly matches the offsets \sfrac{0}{1} and \sfrac{3}{4}, while the
\sfrac{4}{4} meter after it matches the \sfrac{5}{4} offset against its third
beat. The next meter's end-beat offset aligns against the offset counter's
\sfrac{5}{2} offset allowing the meter fitting process to perfectly match the
remaining offsets against a pair of \sfrac{5}{4} meters:
...
permitted to immediately repeat. The above example ends with two \sfrac{5}{4}
...
to 1, the meter fitting process can no longer select two \sfrac{5}{4} meters
for the end of the fitted meter sequence. Instead, a pair of \sfrac{3}{4} and
\sfrac{4}{4} meters replace the previously final \sfrac{5}{4} meter:
...
several offsets with strong simultaneities, notably at \sfrac{11}{8},
\sfrac{13}{8}, \sfrac{17}{8} and \sfrac{25}{8}:
...
against strong beats in each meter. The initial \sfrac{6}{8} meter receive a
2-count accent on its half-way \sfrac{3}{8} offset. The second \sfrac{6}{8}
meter is matches strong accents on both its downbeat, its half-beat and on its
end-beat. Likewise, the \sfrac{7}{8} meter matches strong accents on its
...
\noindent Removing the \sfrac{5}{8} meter from the permitted meters inventory
gives less convincing results. Notably, the strong simultaneities at offsets
\sfrac{11}{8} and \sfrac{17}{8} no longer align with any downbeats in the
fitted meters:
...
results. Both the \sfrac{11}{8} and \sfrac{17}{8} offsets still align with weak
beats in the fitted meters. The only real change is the swapping of
\sfrac{6}{8} and \sfrac{3}{4} meters in the first and last pairs of fitted
meters:
...
example. Not only do the \sfrac{11}{8} and \sfrac{17}{8} offsets align at
downbeats, but the three-weight offset at \sfrac{25}{8} matches the downbeat of
a \sfrac{3}{4} meter with two-weight accents on both its second and third beat
...
initial \sfrac{9}{8} here -- often diminishes the choices possible afterward:
...
in a \sfrac{4}{8} measure is rewritten with a meter of \sfrac{5}{8}, not of
\sfrac{4}{8}:
...
can be reflected around its axis and stretched from a duration of \sfrac{19}{4}
to \sfrac{25}{4}. This will change the overall duration and phrase density of
