...
absolutely along a timeline. This means that every timespan describes a range
...
\texttt{timespantools} subpackage. The following code shows the definition of a
...
\texttt{start\_offset} and \texttt{stop_offset} properties into explicit
...
\texttt{timespantools} subpackage provides an \texttt{AnnotatedTimespan} class
...
annotated timespans from old ones, replacing one annotation with another while
...
A timespan inventory can be created with a list of zero or more
...
iterable object -- by passing the first timespan inventory as an
