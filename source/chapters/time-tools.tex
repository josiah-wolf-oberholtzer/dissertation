...
Talea -- infinitely cyclic patterns of durations.
...
\begin{markdown}
- rhythm-makers mirror timespan-makers
- they come in a variety of classes
- they are highly configurable via keywords
- their configuration is clarified through specifier classes
- specifier classes group related configuration properties together
- recall that keys in nested configuration can still be changed via `new()`
- they act as frozen functions
- they all have an identical interface for input and output
\end{markdown}

\subsection{Note rhythm-makers}

Like many objects implemented in Abjad and its extensions, rhythm-makers can be
illustrated via a call to \texttt{show()}. Rhythm-maker illustrations take an
optional \texttt{divisions} argument, specifying what durations should be used
for the rhythm output, grouping each of those divisions into measures for ease
of understanding.
...
splitting durations across division boundaries when overful. The following
trivial talea rhythm-maker uses a length-1 talea comprised of a single 1/16
duration.
...
the pattern of split and tied durations changes.
...
Extending the even-division rhythm-maker's denominator sequence to a 2-length
pattern produces divisions filled alternatingly with 1/8 and 1/16 notes. Note
that the 3/16 measure is filled with a dotted 1/8 note.
...
Extending the denominator sequence even further results in more complex output.
Note here how the 5/8 division is rendered as a tuplet.
...
Like the talea rhythm-maker, even-division rhythm-makers can be configured with
a \texttt{extra\_counts\_per\_division} property, forcing the rhythm-maker to
treat input divisions as longer than the actually. Artificially lengthened
divisions are rendered as tuplets. With the following rhythm-maker, every other
division is extended by one count, causing the 4/8 and 2/4 divisions to be
rendered as 5:4 tuplets, and the 4/16 division to be rendered as a 3:2 tuplet.
...
When configured with a tie specifier, this even-division rhythm-maker obscures
the downbeat of every output division.
...
And when reconfigured with the earlier 8-4-16 denominator pattern, this
rhythm-maker produces rich tupletted rhythmic output.
...
\subsection{Tuplet rhythm-makers}
