...
\noindent A wide variety of other time relations are also possible, such as
testing for intersection with single offsets. These predicates make possible
many of the generative operations carried out on timespans.

\subsection{Operations on timespans} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Many \texttt{Timespan} methods transform and return new timespans.
...
Splitting generates a timespan inventory, discussed later.

\begin{comment}
<abjad>
result = timespan.split_at_offset(5)
print(format(result))
</abjad>
\end{comment}
...
More complex operations between timespans which result in timespan inventories
include subtraction, unioning and intersection.
...
\begin{comment}
<abjad>
result = timespan_1 - timespan_1
print(format(result))
result = timespan_1 - timespan_2
print(format(result))
result = timespan_1 - timespan_3
print(format(result))
result = timespan_2 - timespan_1
print(format(result))
result = timespan_2 - timespan_2
print(format(result))
result = timespan_2 - timespan_3
print(format(result))
result = timespan_3 - timespan_1
print(format(result))
result = timespan_3 - timespan_2
print(format(result))
result = timespan_3 - timespan_3
print(format(result))
</abjad>
\end{comment}
...
Logical \texttt{OR} results in a unioning of the two input timespans, if they
overlap.
...
Logical \texttt{AND} results in the intersection of the two input timespans:
...
Logical \texttt{XOR} -- exclusive \texttt{OR} -- results in the symmetric
difference of the two input timespans:
...
together a collection of \texttt{Timespan} objects.

In addition to the protocol
defined for ordered collections, timespan inventories provide a variety of
other methods and properties for working specifically with timespans.
...
\subsection{In-place logical operations} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
