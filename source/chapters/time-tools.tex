...
\begin{markdown}
Consort provides a family of factory classes for producing timespan
inventories.

-   the timespan-maker interface:
    -   input: a mapping of voice-names to music specifiers
    -   input: a target timespan (where along our time canvas are we going to
        create timespans?)
    -   input: an optional layer (which pass is this?)
    -   input: an optional timespan inventory to populate
    -   output: a timespan inventory, the same (in id) if one was provided
-   why provide voice names?
    -   a brief discussion of aurora, and exploding timespans to match an
        ensemble (how many inventories to explode to? how to handle any
        overlap? what about ordering?)
    -   obviously, a family of timespan-makers could be designed for
        non-contexted events, especially for granular synthesis textures
    -   however, these timespans are always going to end up being used to
        create scores with named voices. pre-naming saves a step.
    -   the voice-name mapping can be ordered (python dictionaries are not
        ordered in a predictable way, due to their hashing algorithm)
-   independent vs dependent: consort will use these at different points when
    it builds a score
-   timespan specifier: allows the timespan maker to configure some of the
    properties of the performed timespans it creates
-   padding: allows the timespan maker to force "silence" around the beginning
    and ends of the groups of timespans it creates (this will be demonstrated
...
\begin{markdown}
-   flooded timespan-makers are the simplest
-   the name comes from the ubiquitous graphic-design "flood-fill" tool, which
    dates back decades
-   the goal is to create one timespan for each voice in the input
    voice-name-to-music-specifier mapping, which "floods" the target timespan
\end{markdown}
...
Added a second entry to the music specifier mapping results in two timespans in
the output. Likewise, calling the timespan-maker with a \texttt{layer} keyword
configures the output timespans with that layer number. The layer is indicated
in the illustration just above and to the right of the beginning of each
timespan.
...
Configuring the timespan-maker with padding creates silent timespans around the
beginning and end of each group of output timespans on a per-voice basis. The
timespan-maker will also configure these silent timespans with any specified
layer number. Silent timespans are illustrated with dashed line, for clarity.
...
\begin{markdown}
-   creates timespans based off *talea* -- infinitely cyclic duration patterns
-   they can create in a synchronized or unsynchronized fashion
    -   unsynchronized creation goes voice by voice through the music
        specifiers mapping, creating timespans from the start of the target
        timespan to its stop, then wraps around to the next voice
    -   unsynchronized creation is ideal for creating textures
    -   management of silences and groupings can create very dense or very
        sparse textures
    -   synchronized creation is ideal for creating moments of shared attack
        across voices, or for simply guaranteeing that material in some
        collection of voices will overlap for a moment
-   talea control:
    -   the durations of the timespans in each group
    -   the durations of the silences between groups
    -   the durations of the silences at the very beginning of a voice's
        timespans
\end{markdown}

Abjad implements Talea as a \texttt{Talea} class in its
\texttt{rhythmmakertools} subpackage. Talea define a sequence of durations in
terms of a finite sequence of numerators paired with a denominator. Once
defined, they can be iterated over and indexed like an infinite sequence.
...
\noindent All of the timespan inventories created in the following talea
...
\begin{markdown}
Dependent timespan-makers create timespans based on the contents on the
disposition of timespans in an input timespan inventory.

-   inspects voice names
-   inspects labels
-   collects valid timespans into a timespan inventory, partitions and collects
    offsets (optionally including internal starting and stopping offsets)
-   this lets us model creating a piano pedal voice based on timespans
    representing the left and right-hand on-keyboard-only playing
-   or to create the off-hand voice for a split-staff string instrument
    notation, given either the right-hand or left-hand voice-associated
    timespans
-   rotation of durations in the output is also possible
-   rotated timespans occur in the same overall timespan, but have potentially
    different internal structure than the voice-associated timespans they
    depend upon
\end{markdown}
