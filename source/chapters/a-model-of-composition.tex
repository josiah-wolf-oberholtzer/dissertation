...
repeated cycle containing three distinct stages: \emph{specification},
\emph{interpretation} and \emph{visualization}. What follows is a detailed
analysis of the various algorithms and subroutines employed during Consort's
specification and interpretation stages. Visualization, via Abjad's
\emph{illustration protocol} was already discussed in
\autoref{chap:a-model-of-notation}.
...
\sfrac{3}{4} meters permitted, a score-template comprised of two rhythmic
...
more restricted output. Note that the unlabeled \sfrac{6}{1}-\sfrac{8}{1}
timespan in \enquote{Voice 1} and the \sfrac{10}{1}-\sfrac{25}{2} timespan in
...
grid such as \sfrac{1}{8}, \sfrac{1}{16} or \sfrac{1}{32}. Because
...
\sfrac{1}{3} or \sfrac{15}{7} rather than \sfrac{1}{4} or 0 -- will cause all
...
\sfrac{1}{2}, \sfrac{1}{4} or \sfrac{1}{8}. By specifying a timespan
...
\sfrac{0}{1} to \sfrac{19}{4} in voices \enquote{Voice 2} and \enquote{Voice
...
texture only spans the target timespan of \sfrac{3}{4} to \sfrac{19}{4},
guaranteeing that the background layer for \enquote{Voice 2} and \enquote{Voice
3} is untouched in the span of \sfrac{0}{1} to \sfrac{3}{4}:
...
\sfrac{1}{4}-duration silences around the beginning and end of each group,
...
a target timespan of \sfrac{6}{4} to \sfrac{21}{4}. While -- due to the
complexities of the talea timespan-maker's patterns -- the generated timespan
texture may not extend all of the way to its target timespan's stop offset at
\sfrac{21}{4}, it will certainly not contain any performed or silent timespans
earlier than \sfrac{6}{4}, leaving lower layers untouched from \sfrac{0}{1} to
\sfrac{6}{4}:
...
\sfrac{1}{8}-duration -- may give undesirable results. Note that silent
...
which, when passed a division sequence of seven \sfrac{1}{4}-durations,
generates a rhythm consisting of two groups of \sfrac{1}{4} notes delimited by
rests:
...
initial duration of \sfrac{21}{4}, and therefore pads out the end of each
...
2-length logical ties consisting of matched pairs of \sfrac{1}{16} or
\sfrac{1}{32} notes. This takes care of some possible artifacts of heavily
...
the initial \sfrac{1}{16} note in the upper staff -- does not receive the
expected single grace note. The grace note pattern starts instead on the
\sfrac{1}{8} note that follows. For purely practical reasons, Consort's
...
following short segment, containing two voices of \sfrac{1}{16} note rhythms
...
off. For example, the A4 \sfrac{1}{16} note in the lower voice in measure one
is continued by the B4 \sfrac{1}{8} note in the upper voice at the beginning of
measure two, and followed by the C5 \sfrac{1}{8} note directly below in the
...
applying an operation to the first \sfrac{1}{4}, no operation to the middle
\sfrac{1}{2}, and another operation to the final \sfrac{1}{4} of the segment:
...
\noindent Now, consider again the \sfrac{1}{16} note music specifier from
