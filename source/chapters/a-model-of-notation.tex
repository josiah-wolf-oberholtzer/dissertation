...
\todo[inline]{\textbf{TODO:} Namespaces.}
...
\begin{lstlisting}
>>> print(foo)
<__main__.Foo object at 0x103a9b210>
\end{lstlisting}
\begin{lstlisting}
>>> repr(foo)
'<__main__.Foo object at 0x103a9b210>'
...
components to retrieve those component, as shown in \autoref{ssec:containers}
and later. Similarly, named components can be retrieved from a score by
...
The simplest iteration technique makes use of Python's \emph{iteration
protocol} by treating Abjad's containers as list-like objects.
...
The component immediately contained by any Abjad container can be iterated over
as though that container were a list:
...
for y in score['Both Staves']:
    y

for z in score['Upper Staff']:
    z
...
Abjad also provides an \emph{iteration agent} -- much like Abjad's component
inspector -- which exposes a variety of iteration methods. Calling the
top-level \texttt{iterate()} function against a score component returns an
...
Perhaps the most fundamental method of score tree traversal is
...
>>> graph(score)
\end{lstlisting}
\noindent\includegraphics[max width=\textwidth,scale=0.5]{assets/graphviz-cd4e4715fc44715a6b9884dff43b2b03.pdf}
\end{singlespacing}
\end{abjadbookoutput}

\noindent In fact, Abjad's score tree graph visualizations rely, like many of
the other more-specialized score tree iteration techniques, on this depth-first
traversal. One of the most common iteration techniques based closely on
depth-first traversal is iteration by class, whereby a score tree is iterated
over recursively and only those items matching a component class prototype --
e.g. only rests, only tuplets, or only notes and chords -- are yielded. We can
demonstrate this technique by iterating over the same score by rests and
attaching some markup to each yielded rest:
...
Iteration by \emph{timeline} is another important score traversal technique,
yielding each component -- optionally filtered by a class prototype -- in the
order in which they in time relative to the start of the score. Components with
identical start times are yielded according to their score index -- a tuple of
indices describing the index of each of the components in a component's
parentage in \emph{their} parents. We can demonstrate this iteration technique
by iterating over all of the leaves in time-wise order, attaching markup to
display their index in the timeline:
...
Composers are often concerned with contiguous \emph{runs} of notes and chords
rather than any other collection of components Components can use the iteration
agent to iterate over components grouped contiguously by class:
...
Finally, score trees can be traversed by logical ties -- selections
...
With the score prepared, all pitched logical ties in the upper staff can be
iterated over and markup attached to the head of each:
...
selection, or selecting items whose length is longer than some count.

Selectors allow composers to model the procedure by which they, for example,
select rest-delimited runs of notes and chords, then select all logical ties
within those runs and finally select the last leaf of each logical tie, even if
that logical tie contains only a single leaf. Such a procedure, once codified
in a fully-configured selector, can be applied against any leaf, container or
selection of components.

An unconfigured selector simply, when called against a component, simply
selects that component, returning the selection wrapped within a sequence so
that the result can be iterated over.
...
The selector can be configured by calling one of its various generating
methods, which duplicate the selectors current sequence of callbacks and append
a new one, returning a new selector with the modified callback sequence.
...
The selector can be configured to select all pitched logical-ties from with the
previous selections.
...
The selector can be configured to filter out logical ties shorter than a
duration of \sfrac{1}{4}:
...
The first item from each selected logical-tie can then be selected by using
Python's index subscripting syntax.
...
Note that the returned selections each contain a single note. This is not
necessarily useful yet. By appending a single additional flattening callback to
the selector's callback stack, the selector can be configured to return a flat
selection of components which can be iterated over:
...
Unlike component iterators, selectors can be re-used. The same selector which
was called against the upper staff in the above example can be called again,
returning the same result. As with the examples in \autoref{ssec:iteration}, we
can label each component returned by the selector with some markup. Because the
score was already labeled -- marking the head of each logical tie in the upper
staff -- we first remove all markup by iterating depth-first:
...
Printing the storage format of the selector shows its deeply-nested
configuration.
...
\todo[inline]{\textbf{TODO:} Demonstrate storage formatting of selectors.}

\section{Templating}

\todo[inline]{\textbf{TODO:} Explain templating and new().}

One of the last, but most important aspects of working with Abjad does not
concern modeling notation at all.

The act of \emph{templating} takes one object and replaces the values assigned
...
Abjad's top-level \texttt{new()} function enacts templating.
