...
\begin{lstlisting}
>>> print(foo)
<__main__.Foo object at 0x10392d1d0>
\end{lstlisting}
\begin{lstlisting}
>>> repr(foo)
'<__main__.Foo object at 0x10392d1d0>'
...
Indicators attach to single score components.

Indicators are generally immutable, like integers and durations. In being
attached to a score component they do not receive a reference to that
component, allowing the same indicator to be attached to many components.
Instead, Abjad binds the indicator to the component with an
\texttt{IndicatorExpression}, which holds the necessary references along with
other important information about the behavior of the attachment.

\subsection{Attachment}

-   Discuss common indicators: Articulation, BarLine, Clef, Dynamic,
    KeySignature, Markup, Tempo, TimeSignature

-   Demonstrate attach(), detach()

-   Demonstrate inspect\_(...).get\_indicator(), .get\_indicators()

-   Demonstrate inspect\_(...).has\_indicator()

\subsection{Indicator scope}

-   Demonstrate implicit (default) scoping and explicit scoping

-   Clefs per staff

-   Time signatures within each measure, already attached implicitly

-   Score-scope tempo

-   An explicitly-scoped object: Marimba at the StaffGroup scope

-   Demonstrate: inspect\_(...).get\_effective(),
    inspect\_(...).has\_effective\_indicator()
...
-   Demonstrate a non-formatting indication: attach(x, y, is\_annotation=True)

-   Demonstrate an annotated formatting indication

-   Discuss where formatting comes from: LilyPond format bundles

-   Idiomatic indicators: BowContactPoint, IsAtSoundingPitch, IsUnpitched,
    StringContactPoint, StringTuning, etc.
