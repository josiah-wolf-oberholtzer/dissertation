...
\begin{minted}{pycon}
>>> print(foo)
<__main__.Foo object at 0x104abed50>
\end{minted}
\begin{minted}{pycon}
>>> repr(foo)
'<__main__.Foo object at 0x104abed50>'
...
LilyPond output always shows a \sfrac[big]{4}{4} time signature even though none of
the created leaves takes up a whole note's duration. LilyPond simply treats all
music as \sfrac[big]{4}{4} unless explicitly instructed otherwise. We can make the
\sfrac[big]{4}{4} time signature explicit by instantiating a \sfrac[big]{4}{4} measure
...
\noindent Iterating over the contents of the \sfrac[big]{4}{4} measure yields only
...
its triplet grandparent and the \sfrac[big]{4}{4} measure as its
...
e.g. \sfrac[big]{1}{2}, \sfrac[big]{7}{23} or \sfrac[big]{16}{1}. This concern arises from the
...
properties. For example, a \sfrac[big]{7}{16} duration can be instantiated from a
...
used to instantiate a leaf. A \sfrac[big]{7}{16} duration, without resorting to
...
on LilyPond's default \sfrac[big]{4}{4} time signature and treble clef:
...
than a duration of \sfrac[big]{1}{4}:
