%!TEX TS-program = xelatex
%!TEX encoding = UTF-8 Unicode

\documentclass[oneside,School=Harvard]{Dissertate}

\geometry{
    a4paper,
    margin=1in,
    %showframe=true,
}

\usepackage[color=yellow]{todonotes}
\usepackage[export]{adjustbox}
\usepackage[htt]{hyphenat}
\usepackage{color}
\usepackage{csquotes}
\usepackage{dirtree}
\usepackage{excludeonly}
\usepackage{fancyvrb}
\usepackage{float}
\usepackage{imakeidx}
\usepackage{lipsum}
\usepackage{listings}
\usepackage{microtype}
\usepackage{minted}
\usepackage{multicol}
\usepackage{pdfpages}
\usepackage{textcomp}
\usepackage{upquote}
\usepackage{xfrac}

\usepackage[
    labelfont={sc,scriptsize},
    textfont={rm,scriptsize},
    singlelinecheck=off,
]{caption}

%\usepackage[margincaption,outercaption,raggedright]{sidecap}
%\sidecaptionvpos{figure}{t} 
%\sidecaptionvpos{table}{t}

\usepackage{sidenotes}

\newenvironment{abjadbookoutput}{}{}

\newenvironment{markdown}{
    \VerbatimEnvironment
    \begin{VerbatimOut}{tmp/tmp.markdown}%
}{  
    \end{VerbatimOut}
    \immediate\write18{pandoc tmp/tmp.markdown -t latex -o tmp/tmp.tex}
    \input{tmp/tmp.tex}
}

\setmonofont{Inconsolata}[Scale=MatchLowercase]

\setminted{
    autogobble=true,
    fontsize=\footnotesize,
    frame=leftline,
    framerule=1.5pt,
    framesep=6pt,
    obeytabs=true,
    tabsize=4,
    xleftmargin=-6pt,
}

\includeonly{
    source/chapters/introduction,
    source/chapters/a-model-of-notation,
    source/chapters/time-tools,
    source/chapters/a-model-of-composition,
    source/chapters/practicalities,
    source/chapters/conclusion,
    source/endmatter/colophon
}

%\excludeonly{
%    source/chapters/score-aurora,
%    source/chapters/score-plague-water,
%    source/chapters/score-zaira,
%    source/chapters/score-armilla,
%    source/chapters/score-ersilia,
%    source/appendices/appendices,
%}

\makeindex[columns=3, title=Index, intoc]

\makeatletter
\def\cleardoublepage{\clearpage\if@twoside%
\ifodd\c@page\else
\vspace*{\fill}
\hfill
\begin{center}
This page intentionally left blank.
\end{center}
\vspace{\fill}
\thispagestyle{fancy}
\newpage
\if@twocolumn\hbox{}\newpage\fi\fi\fi
}
\makeatother

\makeatletter
\def\cleardoublepageempty{\clearpage\if@twoside%
\ifodd\c@page\else
\vspace*{\fill}
\hfill
\vspace{\fill}
\thispagestyle{empty}
\newpage
\if@twocolumn\hbox{}\newpage\fi\fi\fi
}
\makeatother

\raggedbottom

%\usepackage{ebgaramond}
%\renewcommand{\LettrineFontHook}{\fontfamily{EBGaramond-Initials}\selectfont}

\usepackage{ebgaramond}

\newcommand{\eblettrine}[3][]{%
\lettrine[findent=6pt,lines=3,depth=1,lhang=0.,loversize=0.,slope=0pt,nindent=0pt]
{\textin{\textcolor{SchoolColor}{#2}}}{#3}%
}

\newcommand{\eblettrinedbl}[3][]{%
\lettrine[findent=10pt,lines=2,depth=1,lhang=0.,lraise=-0.30,loversize=0.40,slope=0pt,nindent=0pt]
{\textin{\textcolor{SchoolColor}{#2}}}{#3}%
}

\begin{document}

\frontmatter
\pagenumbering{roman}
\input{source/frontmatter/personalize}

\includepdf[
    trim=0.5in 0.5in 1in 0.5in,
    pages=-,
    pagecommand={\thispagestyle{empty}}
]{assets/dac-oberholtzer-2015.pdf}

\cleardoublepageempty

\maketitle
\copyrightpage
\abstractpage
\tableofcontents
%\listoffigures
\dedicationpage
\acknowledgments

\mainmatter
\doublespacing
\cleardoublepage
\pagenumbering{arabic}
\setcounter{page}{1}

\part{Theory}

\include{source/chapters/introduction}
\include{source/chapters/a-model-of-notation}
\include{source/chapters/time-tools}
\include{source/chapters/a-model-of-composition}
\include{source/chapters/practicalities}
\include{source/chapters/conclusion}

\part{Practice}

\include{source/chapters/score-aurora}
\include{source/chapters/score-plague-water}
\include{source/chapters/score-zaira}
\include{source/chapters/score-armilla}
\include{source/chapters/score-ersilia}

\part{Appendices}

\lstset{
    basicstyle=\scriptsize\ttfamily,
    breakatwhitespace=false,
    breaklines=false,
    commentstyle=\color{greencomments},
    escapeinside={(*@}{@*)},
    keywordstyle=\color{bluekeywords}\bfseries,
    language=Python,
    frame=none,
    numbers=left,
    numbersep=5pt,
    numberstyle=\tiny\color{grey},
    showspaces=false,
    showstringspaces=false,
    showtabs=false,
    stringstyle=\color{redstrings},
    upquote=true,
}

\singlespacing
\begin{appendices}
    \include{source/appendices/appendices}
\end{appendices}

\cleardoublepage
\printindex

\maketheglossary

\cleardoublepage
\addcontentsline{toc}{chapter}{References}
%\nocite{*}
\bibliographystyle{apalike2}
\bibliography{references}

\include{source/endmatter/colophon}

\end{document}